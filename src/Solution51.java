...
public class Solution51 {
    ...
    public static int[] solution(int[][] m){
        setIntMatrix(m);
        setRatMatrix();
        identityMatrix = getIdentity(intMatrix);
        rMatrix = getSubR(intMatrix);
        qMatrix = getSubQ(intMatrix);
        return calculateFR();
    }
    ...
    public static void setIntMatrix(int[][] arr){
        intMatrix = sortM(arr);

    }
    public static void setRatMatrix(){
        ...
        toIdentity(temp);
        ratMatrix = new Rat[intMatrix.length][intMatrix.length];
        for(int i=0;i< intMatrix.length; i++){
            for(int j=0;j< intMatrix.length; j++){
                ratMatrix[i][j] = new Rat(temp[i][j], getDen(temp[i]));
    ...
    public static Rat[][] getSubQ(int arr[][]){
        int counter = 0;
        for (int i=0; i<arr.length; i++){
            if(isTerminal(arr[i])){
                counter++;
        ...
        int dim = arr.length-counter;


        Rat[][] result = new Rat[dim][dim];
        for(int j=0; j< dim;j++){
            for(int k =0; k< dim;k++){
                result[j][k] = ratMatrix[j+arr.length-dim][k+arr.length-dim];
            }

        }
        return result;
    }
    public static Rat[][] getSubR(int arr[][]){
        int counter = 0;
        for (int i=0; i<arr.length; i++){
            if(isTerminal(arr[i])){
                counter++;
        ...
        int dimX = counter;
        int dimY = arr.length-counter;

        Rat[][] result = new Rat[dimY][dimX];
        for(int j=0; j< dimY;j++){
            for(int k =0; k< dimX;k++){
                result[j][k] = ratMatrix[j+arr.length-dimY][k];
            }

        }
        return result;
    }
    public static Rat[][] getIdentity(int arr[][]){
        int counter = 0;
        for (int i=0; i<arr.length; i++){
            if(isTerminal(arr[i])){
                counter++;
        ...
        Rat[][] result = new Rat[counter][counter];
        for(int j=0; j<counter;j++){
            for(int k =0; k< counter;k++){
                result[j][k] = ratMatrix[j][k];
    ...
    public static int[][] toIdentity(int arr[][]){
        for(int i =0; i< arr.length; i++){
            if(isTerminal(arr[i]))arr[i][i] = 1;
        }
        return arr;
    }
    public static int[][] sortM(int arr[][]){
        ...
        for(int i=1;i<arr.length;i++){
            if(!isTerminal(arr[i])) {
                for (int j = 0; j < arr.length; j++) {
                    if (!isTerminal(arr[j]) && getDen(arr[i]) < getDen(arr[j])) {
    ...
    public static int[][] swapArr(int i, int j, int[][] arr){
        int temp[] = arr[i];
        arr[i] = arr[j];
        arr[j] = temp;
    ...
    public static int[] swapInner(int i, int j, int[] arr){
        int temp = arr[i];
        arr[i]= arr[j];
        arr[j] = temp;
    ...
    private static boolean isTerminal(int arr[]){
